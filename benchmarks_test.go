...
func BenchmarkRegexExpression(bench *testing.B) {
	expression, _ := NewEvaluableExpression("(foo !~ bar) && (foobar =~ oba)")
	parameters := map[string]interface{}{
		"foo": "foo",
		"bar": "bar",
		"baz": "baz",
...
func BenchmarkConstantRegexExpression(bench *testing.B) {
	expression, _ := NewEvaluableExpression("(foo !~ '[bB]az') && (bar =~ '[bB]ar')")
	parameters := map[string]interface{}{
		"foo": "foo",
		"bar": "bar",
	}
...
func BenchmarkAccessors(bench *testing.B) {
	expression, _ := NewEvaluableExpression("foo.Int")
...
func BenchmarkAccessorMethod(bench *testing.B) {
	expression, _ := NewEvaluableExpression("foo.Func()")
...
func BenchmarkAccessorMethodParams(bench *testing.B) {
	expression, _ := NewEvaluableExpression("foo.FuncArgStr('bonk')")
...
func BenchmarkNestedAccessors(bench *testing.B) {
	expression, _ := NewEvaluableExpression("foo.Nested.Funk")
